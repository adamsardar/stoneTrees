...
glpktim = system.time(
  SteinForGLPK = nodeCentricSteinerForestProblem$new(
    fixedTerminalLymphomaGraph,
    solverChoice = "RGLPK"
  )$sampleMultipleBootstrapSteinerSolutions(nBootstraps = 100, maxItr = 5)
)
## Takes around a minute using RGLPK as the solver
cbctime = system.time(
  SteinForCBC = nodeCentricSteinerForestProblem$new(
    fixedTerminalLymphomaGraph,
    solverChoice = "RCBC"
  )$sampleMultipleBootstrapSteinerSolutions(nBootstraps = 100, maxItr = 5)
)
...
testSteinFor = nodeCentricSteinerForestProblem$new(
  fixedTerminalLymphomaGraph,
  verbose = FALSE,
  solverChoice = "rcbc"
)
...
solDetailsDT %>%
  ...
  labs(
    x = "Solver",
    y = "Time (minues) [log scale]",
    title = "Comparison of CBC and GLPK solving single iterations ofthe Steiner Forest problem"
  )

solDetailsDT[, .N, by = .(RCPCvcount <= RGLPKvcount)]
solDetailsDT[, .N, by = .(RCPCvcount >= RGLPKvcount)]

system.time(nodeCentricSteinerForestProblem$new(
  fixedTerminalLymphomaGraph,
  verbose = FALSE,
  solverChoice = "rcbc"
...
bench = microbenchmark(
  {
    ...
    nodeCentricSteinerForestProblem$new(
      ...
      verbose = FALSE,
      solverChoice = "rcbc"
    )$sampleMultipleBootstrapSteinerSolutions()$getBootstrapSolutionPoolGraphs()
  },
  {
    set.seed(1234)
    nodeCentricSteinerForestProblem$new(
...
for (i in 1:100) {
  ...
  #Ensure that our solvers are both solving the same type of problem
  # We can't just run two bootstrap Steiner forest routines
  rglpkTime = system.time(
    RGLPKsol = nodeCentricSteinerForestProblem$new(
      fixedTerminalLymphomaGraph,
      verbose = FALSE,
      solverChoice = "rglpk"
  ...
  rcbcTime = system.time(
    RCBCsol = nodeCentricSteinerForestProblem$new(
      fixedTerminalLymphomaGraph,
      verbose = FALSE,
      solverChoice = "rcbc"
    )$sampleMultipleBootstrapSteinerSolutions()
...
fwrite(
  ...
  "~/steinForestSuboptimalBench.tsv",
  sep = "\t"
)
...
for (i in 1:15) {
  ...
  cbcSizes = sapply(
    SteinForCBC$getBootstrapSolutionPoolGraphs(collapseSols = FALSE),
    vcount
  )
  glpkSizes = sapply(
    SteinForGLPK$getBootstrapSolutionPoolGraphs(collapseSols = FALSE),
    vcount
  )
  ...
  fwrite(
    ...
    file = "~/steinForestSuboptimalBench.tsv",
    sep = "\t",
    quote = FALSE,
    append = TRUE
  )
...
GLPKmodules = steinForestSuboptimalBenchDT[,
  .(solver = "GLPK", size = unlist(strsplit(GLPKmodulesVcount, "\\|"))),
  by = trial
]
CBCmodules = steinForestSuboptimalBenchDT[,
  .(solver = "CBC", size = unlist(strsplit(CBCmodulesVcount, "\\|"))),
  by = trial
]
...
solutionModules %>%
  ...
  labs(
    x = "solver",
    y = "Module size (nodes) of a single Steiner forest sub-problem",
...
solutionModules[, .N, by = .(trial, solver)] %>%
  ...
  theme_bw(base_size = 20) +
  labs(
    title = "Size of solution pool (i.e. the number of distinct solutions to Steiner forest subproblems) collected",

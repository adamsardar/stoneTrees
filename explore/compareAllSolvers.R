    ...
    safely(
      ~ {
        ...
        save(
          SolutionPool,
          file = paste0(
            "./results/steinForestSuboptimalSolver_",
            .x,
            "_Trial",
            i,
            ".RData"
...
steinForestSuboptimalBenchDT <- fread(
  "./results/steinForestSuboptimalBench.tsv"
) %>%
...
p4 <- steinForestSuboptimalBenchDT[,
...
plot <- egg::ggarrange(
  ...
  top = paste(
    ...
    "runs with",
    bootstrapIteraction,
    "bootstraps, collecting up to",
    nsuboptimalSol,
    "degenerate solutions on each run."
  )
)
...
ggsave(
  plot = plot,
  filename = "./results/CBCvsCPLEXvcGPLKPerformance.png",
  width = 16,
  height = 15
)

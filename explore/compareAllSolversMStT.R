...
MStTSuboptimalBenchDT <- map(1:numberOfTrial, function(i) {
  ...
  Results <- map(
    stoneTrees_solvers[1:3],
    safely(
      ~ {
        #lpsymphony is creating issues

        message(paste("Starting", .x))

        time <- system.time(
          SteinForSolverX <- subOptimalSteinerProblem$new(
            fixedTerminalLymphomaGraph,
            solverChoice = .x,
            verbose = TRUE,
            solverTrace = 0
          )$identifyMultipleSteinerSolutions(maxItr = nsuboptimalSol)
        )

        SolutionPool <- SteinForSolverX$getSolutionPoolGraphs(
          collapseSols = FALSE
        )
        ...
        save(
          SolutionPool,
          file = paste0("./results/MStTSolver_", .x, "_Trial", i, ".RData")
        )
...
MStTSuboptimalBenchDT[
  trial == 1,
  .(solver, time, vcount, NSolutions = Niteration)
]

MStTSuboptimalBenchDT[,
  .(`TotalTime(hours)` = sum(time) / 60 / 60),
  by = .(solver)
]

MStTSuboptimalBenchDT[, .(TotalTime = sum(time) / 60 / 60), by = .(solver)][,
  TotalTime
] %>%
  sum
...
p1 <- ggplot(MStTSuboptimalBenchDT, aes(x = solver, y = time / 60)) +
  ...
  labs(y = "Time (min)", title = "Time taken to solve Steiner forest problem")
...
p3 <- ggplot(MStTSuboptimalBenchDT, aes(x = solver, y = vcount)) +
  ...
  #scale_color_hue()+
  theme(legend.position = "none") +
  labs(y = "Aggregated module size", title = "Size of Steiner forest solution")


p4 <- MStTSuboptimalBenchDT[,
  .(modulesVcount = unlist(modulesVcount)),
  by = .(solver, time, vcount, Niteration, trial)
] %>%
  ...
  theme(legend.position = "none") +
  labs(
    y = "vCount of all modules in the pool",
...
p5 <- ggplot(MStTSuboptimalBenchDT, aes(x = solver, y = Niteration)) +
  ...
  theme_bw(base_size = 20) +
  theme(legend.position = "none") +
  labs(y = "Number of solutions", title = "Size of solution pool")
...
ggsave(
  plot = plot,
  filename = "./results/CBCvsCPLEXvcGPLKPerformanceMStT.png",
  width = 16,
  height = 15
)

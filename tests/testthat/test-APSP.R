...
test_that("Look at APSP for gene42", {
  ...
  expect_true(is_igraph(gene42APSPmod))
  expect_true(is_connected(gene42APSPmod))
  expect_error(calculateAPSPmodule(
    c("not a", "set", "of", "seeds"),
    gene42_igraph
  ))
  expect_warning(calculateAPSPmodule(
    c(seedNodes, "not a", "set", "of", "seeds"),
    gene42_igraph
  ))
...
test_that("Check Behaviour On A Graph With NA Values - with a seed set containing an NA node", {
  ...
  expect_true(is_igraph(apspMod))
  expect_true(is_connected(apspMod))

  expect_false(
    any(is.na(V(apspMod)$name)),
    info = "There shouldn't be any NA nodes in the answer here (looking at the input graph)"
  )
})
...
test_that("Check Behaviour On A Graph With NA Values - with a path known to include an NA-named node", {
  ...
  expect_equal(
    vcount(apspMod),
    6,
    info = "Correct module size is 6, but if too many NA's are included in the subgraph induction step we shall have more nodes that we want"
  )
  expect_equal(
    vcount(induced_subgraph(apspMod, V(apspMod)[is.na(name)])),
    1,
    info = "There should only be one NA named node in this graph"
  )
})

context(
  "Compare results of the default solver, lpSolve, with all other supported solvers available"
)

library(igraph)

steinObject = nodeCentricSteinerTreeProblem$new(
  lymphomaGraph,
  verbose = FALSE,
  presolve = FALSE,
  solverChoice = "LPSOLVE"
)
...
test_that("Compare cplexAPI result", {
  ...
  expect_equal(
    sum(V(lpSolveSolution)$nodeScore),
    sum(V(cplexapiSolution)$nodeScore)
  )
...
test_that("Compare Rglpk result", {
  ...
  expect_equal(
    sum(V(lpSolveSolution)$nodeScore),
    sum(V(glpkSolution)$nodeScore)
  )
  expect_true(is_connected(glpkSolution))
})

test_that("Compare lpsymphony result", {
  if (!"lpsymphony" %in% .packages(all.available = TRUE)) {
    skip("lpsymphony not installed")
  }
  ...
  expect_equal(
    sum(V(lpSolveSolution)$nodeScore),
    sum(V(symphonySolution)$nodeScore)
  )

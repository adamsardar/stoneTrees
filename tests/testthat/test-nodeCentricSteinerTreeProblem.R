...
inspectNodeCentricSteinerTreeObjectCreation = function(testGraph) {
  test_that("Inspect object creation", {
    ...
    expect_message(
      {
        testSteinObject = nodeCentricSteinerTreeProblem$new(
          testGraph,
          verbose = TRUE,
          presolveGraph = FALSE
        )
    ...
    expect_silent(nodeCentricSteinerTreeProblem$new(
      testGraph,
      verbose = FALSE,
      presolveGraph = FALSE
    ))
    ...
    expect_equal(
      dim(testSteinObject$getFixedTerminalConstraints()$variables),
      c(
        nrow(testSteinObject$getNodeDT()[isTerminal == TRUE]),
        vcount(testGraph)
      ),
    ...
    expect_equal(
      nrow(testSteinObject$getFixedTerminalConstraints()$variables),
    ...
    expect_equal(
      testSteinObject$getFixedTerminalConstraints()$directions,
    ...
    expect_equal(
      testSteinObject$getFixedTerminalConstraints()$rhs,
      rep(1, nrow(testSteinObject$getNodeDT()[isTerminal == TRUE])),
      info = "Fixed terminals are simple 1=1 equalities"
    )

    expect_equivalent(
      rowSums(testSteinObject$getFixedTerminalConstraints()$variables),
      rep(1, nrow(testSteinObject$getNodeDT()[isTerminal == TRUE])),
    ...
    expect_equal(
      dim(testSteinObject$getNodeDegreeConstraints()$variables),
    ...
    expect_equal(
      nrow(testSteinObject$getNodeDegreeConstraints()$variables),
    ...
    expect_true(all(
      rowSums(testSteinObject$getNodeDegreeConstraints()$variables) <
        degree(testGraph)
    ))

    expect_true(
      all(
        diag(testSteinObject$getNodeDegreeConstraints()$variables) %in%
          c(-1, -2)
      ),
    ...
    expect_true(
      all(
        diag(testSteinObject$getNodeDegreeConstraints()$variables)[
          testSteinObject$getTerminals()$terminals
        ] ==
          -1
      ),
    ...
    expect_equal(
      dim(testSteinObject$getTwoCycleConstraints()$variables),
      c(
        nrow(merge(
          testSteinObject$getNodeDT()[nodeScore > 0],
          testSteinObject$getEdgeDT(),
          by.x = ".nodeID",
          by.y = "fromNodeID"
        )),
        vcount(testGraph)
    ...
    expect_equal(
      nrow(testSteinObject$getTwoCycleConstraints()$variables),
    ...
    expect_equivalent(
      rowSums(testSteinObject$getTwoCycleConstraints()$variables),
      rep(
        0,
        nrow(merge(
          testSteinObject$getNodeDT()[nodeScore > 0],
    ...
    expect_equivalent(
      testSteinObject$getTwoCycleConstraints()$rhs,
      rep(
        0,
        nrow(merge(
          testSteinObject$getNodeDT()[nodeScore > 0],
    ...
    expect_equivalent(
      testSteinObject$getTwoCycleConstraints()$directions,
      rep(
        ">=",
        nrow(merge(
          testSteinObject$getNodeDT()[nodeScore > 0],
    ...
    expect_true(is.character(
      testSteinObject$getConnectivityConstraints()$directions
    ))
    expect_true(
      length(testSteinObject$getConnectivityConstraints()$directions) == 0
    )

    expect_true(all(
      dim(testSteinObject$getConnectivityConstraints()$variables) ==
        c(0, vcount(testGraph))
    ))

    expect_equal(
      nrow(testSteinObject$getConnectivityConstraints()$variables),
...
test_that("Ensure that incorrect constructor inputs are not tolerated", {
  expect_error(
    nodeCentricSteinerTreeProblem$new(letters),
    regexp = "igraph",
  ...
  expect_error(
    nodeCentricSteinerTreeProblem$new(lymphomaGraph, verbose = "notAboolean"),
    regexp = "boolean",
...
test_that("Studying nodeCentricSteinerTreeProblem solver for correctness of solution against a small MStTP", {
  karateGraph_MSTP = nodeCentricSteinerTreeProblem$new(
    karateGraph,
    verbose = FALSE
...
test_that("Studying nodeCentricSteinerTreeProblem solver for correctness of solution against a medium MStTP", {
  if (!any(c("RCplex", "rcbc") %in% .packages(all.available = TRUE))) {
    skip("SteinLib test takes too long using GLPK. Use CBC or CPLEX.")
  }

  gene42_MSTP = nodeCentricSteinerTreeProblem$new(
    as.undirected(gene42_igraph),
    verbose = FALSE
  )$findSingleSteinerSolution(maxItr = 50)
  ...
  expect_equal(
    graph_attr(gene42_MSTP)$SearchNetwork,
    "as.undirected(gene42_igraph)"
  )
})
...
test_that("Studying nodeCentricSteinerTreeProblem solver for correctness of solution against an easy MWCS", {
  lymphoma_MWCS = nodeCentricSteinerTreeProblem$new(
    lymphomaGraph,
    verbose = FALSE
  )$findSingleSteinerSolution()
...
test_that("Trying MWCS solver against the lymphoma.stp instance in the ACTMOD SteinLib set", {
  skip(
    "SteinLib drosophila test takes too long even using CPLEX - although it does pass!"
  )

...
test_that("Checking the MWCS STP file format reader on SteinLib for ACTMOD problem gene lymphoma.stp", {
  ...
  expect_equal(
    V(lymphoma_igraph)[name == '575']$nodeScore,
    -6.14,
    tolerance = 0.01
  )
  expect_equal(
    V(lymphoma_igraph)[name == '1319']$nodeScore,
    -7.23,
    tolerance = 0.01
  )
  expect_equal(
    V(lymphoma_igraph)[name == '554']$nodeScore,
    -6.94,
    tolerance = 0.01
  )
})
...
test_that("Checking the MWCS STP file format reader on SteinLib for ACTMOD problem gene HCMV.stp", {
  ...
  #Check that nodes have be attributed the correct scores
  expect_equal(
    V(HCMV_igraph)[name == '1359']$nodeScore,
    -3.462,
    tolerance = 0.01
  )
  expect_equal(
    V(HCMV_igraph)[name == '3107']$nodeScore,
    -1.296,
    tolerance = 0.01
  )
  expect_equal(
    V(HCMV_igraph)[name == '551']$nodeScore,
    -1.648,
    tolerance = 0.01
  )
})
...
test_that("Checking the MWCS STP file format from SteinLib for GENE problem gene gene42.stp", {
  ...
  expect_identical(
    length(V(gene42_in)[isTerminal == TRUE]$name),

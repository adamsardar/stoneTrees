...
condenseSearchGraph = function(graphToCondense, condensedNodeSep = ";") {
  check_network(
    graphToCondense,
    singleWeakComponent = FALSE,
    isDirected = FALSE
  )
  check_string(condensedNodeSep, allow_empty = FALSE)
  stopifnot(
    "nodeNameSep must not be in any node names already" = !any(
      V(graphToCondense)$name %like% condensedNodeSep
    )
  )
  ...
  if (!"isTerminal" %in% vertex_attr_names(graphToCondense)) {
    V(graphToCondense)$isTerminal = FALSE
  }
  if (!"nodeScore" %in% vertex_attr_names(graphToCondense)) {
    V(graphToCondense)$nodeScore = -1
  }

  V(graphToCondense)$condensedNode = FALSE

  if (length(V(graphToCondense)[nodeScore > 0 | isTerminal]) == 0) {
    stop("No potential terminals in input graph!")
  }
  ...
  potentialTerminalGraph = induced_subgraph(
    graphToCondense,
    V(graphToCondense)[potentialTerminal]
  )

  nodeDT = as_data_frame(graphToCondense, what = "vertices") %>%
    data.table(key = ".vertexID")
  nodeDT[, segmentID := .vertexID]

  lapply(
    decompose(potentialTerminalGraph, mode = "weak", min.vertices = 2),
    function(g) {
      nodeDT[V(g)$.vertexID, segmentID := min(segmentID)]
      return(invisible(nodeDT[]))
    }
  )

  collapseNodeName = function(x) {
    return(paste0(x, collapse = condensedNodeSep))
  }
  isCollapsed = function(x) {
    return(length(x) > 1)
  }

  condensedGraph = contract(
    graphToCondense,
    mapping = nodeDT[
      V(graphToCondense)$.vertexID,
      as.integer(as.factor(segmentID))
    ], #Create a integer sequence along the segmentID variable
...
uncondenseGraph = function(condensedGraph) {
  ...
  stopifnot(
    "condensedGraph must be an igraph object resultant from running 'condenseSearchGraph'" = 'condensedNode' %in%
      vertex_attr_names(condensedGraph)
  )
  stopifnot(
    "condensedGraph must be an igraph object resultant from running 'condenseSearchGraph'" = 'originalGraph' %in%
      graph_attr_names(condensedGraph)
  )
  stopifnot(
    "condensedGraph must be an igraph object resultant from running 'condenseSearchGraph'" = 'nodeNameSep' %in%
      graph_attr_names(condensedGraph)
  )
  ...
  condensedGraphVertexIDs = V(condensedGraph)[
    condensedNode == TRUE
  ]$.vertexID %>%
  ...
  returnGraph = induced_subgraph(
    originalGraph,
    V(originalGraph)[.vertexID %in% condensedGraphVertexIDs]
  )

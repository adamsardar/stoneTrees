...
parseSTPfile = function(fileName) {
  ...
  nNodes = file.lines %>%
    ...
    str_extract('\\d+') %>%
    as.integer()
  nTerminals = file.lines %>%
    str_match('Terminals\\s+\\d+') %>%
    na.omit() %>%
    str_extract('\\d+') %>%
    as.integer()

  if (length(nEdges) > 0 & length(nArcs) == 0) {
    nArcs = 0
  }
  if (length(nArcs) > 0 & length(nEdges) == 0) {
    nEdges = 0
  }

  if (length(nEdges) > 1) {
    stop(
      'MStTP parser failed. There should only be one line that declares the number of edges'
    )
  }
  if (length(nArcs) > 1) {
    stop(
      'MStTP parser failed. There should only be one line that declares the number of arcs'
    )
  }
  if (length(nNodes) > 1) {
    stop(
      'MStTP parser failed. There should only be one line that declares the number of nodes'
    )
  }
  if (length(nTerminals) > 1) {
    stop(
      'MStTP parser failed. There should only be one line that declares the number of terminals'
    )
  }

  graphEdgesDT = file.lines %>%
    str_match('^[AE]?\\s+\\d+\\s+\\d+\\s*\\d*') %>%
    na.exclude() %>%
    str_split('\\s+') %>%
    data.table(edgeInfo = .)
  graphEdgesDT[,
    edgeVerbosity := length(edgeInfo[[1]]),
    by = 1:nrow(graphEdgesDT)
  ]

  if (graphEdgesDT[, unique(edgeVerbosity)] %>% length > 1) {
    stop('Differing amounts of information in each edge line!')
  }
  ...
  if (nrow(terminalSet) != nTerminals) {
    stop('Terminal count wrong!')
  }
...
readMStTPgraph = function(MStTPstpFile) {
  ...
  if (ncol(edgesDT) == 4) {
    ...
    MStTP_igraph = graph.data.frame(edgesDT[, .(
      start,
      end,
      edgeWeight = weight
    )])
  ...
  V(MStTP_igraph)[
    name %in% stpGraphInformation$terminalNodes$nodeIndex
  ]$isTerminal = TRUE
  ...
  if (length(which(V(MStTP_igraph)$isTerminal)) >= 0.3 * vcount(MStTP_igraph)) {
    warning(
      'Over 30% of your graph nodes are fixed terminals. Are you sure that this is what you want?'
    )
  }
...
readMWCSgraph = function(MWCSstpFile) {
  ...
  if (nrow(terminalDT) < vcount(MWCS_igraph)) {
    warning(
      'It is recommended that you provide a node score per vertex for the MWCS problem'
    )
  }
  ...
  if (!'nodeScore' %in% colnames(terminalDT)) {
    stop(
      'Terminal scores must be present for some of the nodes in the MWCS problem (by definition!)'
    )
  }
  ...
  V(MWCS_igraph)[terminalDT[
    V(MWCS_igraph)$name,
    nodeIndex
  ]]$nodeScore = terminalDT[V(MWCS_igraph)$name, nodeScore]
...
writeMStTPfile_heinzFormat = function(
  network.igraph,
  file.name,
  remark = 'Nothing of note',
  problem = 'MStTP',
  ...
  cat(paste(
    c('Creator "', system('whoami', intern = TRUE), '"', "\n"),
    collapse = ''
  ))
  ...
  if (is.directed(network.igraph)) {
    ...
    write.table(
      edges2ID[order(indexA)][, .(edgeType, indexA, indexB, edgeWeight)],
      file = '',
      quote = FALSE,
      row.names = FALSE,
      col.names = FALSE
    ...
    write.table(
      edges2ID[order(indexA)][, .(edgeType, indexA, indexB)],
      file = '',
      quote = FALSE,
      row.names = FALSE,
      col.names = FALSE
  ...
  write.table(
    node2ID[, .(nodeType, index, terminalPrize)],
    file = '',
    quote = FALSE,
    row.names = FALSE,
    col.names = FALSE

...
calculateAPSPmodule = function(seeds, searchGraph, omitNA = TRUE) {
  ...
  ifelse(
    omitNA,
    seeds %<>% na.omit(seeds),
    if (any(is.na(V(searchGraph)$name))) {
      warning(
        "NA nodes found in input seed list but 'omitNA' set to FALSE - this can lead to unstable behaviour"
      )
    }
  )
  ...
  if (length(seedsNotInGaph) == length(seeds)) {
    stop("None of the seeds were found in graph!")
  }
  if (length(seedsNotInGaph) > 0) {
    warning("Not all seeds found in graph: ", seedsNotInGaph)
  }

  APSP_vertices = map(
    V(searchGraph)[name %in% seedsInGaph],
    ~ {
      all_shortest_paths(
        searchGraph,
        from = .x,
